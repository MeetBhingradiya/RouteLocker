...
function setupLibrary(libraryName: string, generateExample = false) {
    ...
    if (generateExample) {
        ...
        execSync('npx create-react-app react-route-locker-engine --template typescript');
        execSync('echo "SKIP_PREFLIGHT_CHECK=true" >> react-route-locker-engine/.env');
    }
    ...
    if (generateExample) {
        console.log(yellow('Linking packages to the react-route-locker-engine app...'));
        execSync(
            'cd ./react-route-locker-engine && yarn add link:.. link:../node_modules/react link:../node_modules/react-dom && cd ..'
        );
    }
